...
int main()
{
    ...
    {
        cout << "\033[1;31mInvalid login credentials.\033[0m" << endl;
    }
    return 0;
}
...
int calculateTotalBill(CartItem cart[], int cartCount, Item items[], int itemCount)
{
    int total = 0;
    for (int i = 0; i < cartCount; i++)
    {
        ...
        {
            if (cart[i].item.name == items[j].name)
            {
                total += cart[i].quantity * items[j].price;
                break;
            }
        }
    }
...
void confirmPurchase(CartItem cart[], int &cartCount)
{
    int total = calculateTotalBill(cart, cartCount, items, itemCount);
